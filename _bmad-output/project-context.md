...
- **Screen ↔ page map:** 01.2 Login → `pages/auth.py` (`/`, `/login`); 01.1 Register → `register.py`; 01.3 Statement Upload → `upload.py`; 01.5 Dashboard → `dashboard.py`; 01.4 Transactions → `transactions.py`; 01.6 AI Insights → `insights.py`; 01.7 Copilot → `copilot.py`; 02.1 Commitments Management → `commitments.py` (a dedicated page reached from the Dashboard's "+ Add a commitment", **not** the sidebar). The four sidebar screens share the prototype's left-nav (`nav.js` → `.side-nav`).
